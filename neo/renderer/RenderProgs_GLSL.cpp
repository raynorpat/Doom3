...
idStr StripDeadCode( const idStr& in, const char* name )
{
    ...
    while( !src.EndOfFile() )
    {
        ...
        {
            idStr parms, body;
            src.ParseBracedSectionExt( parms, -1, true, '(', ')' );
            ...
            {
                src.ParseBracedSectionExt( body, -1, true, '{', '}' );
                block.postfix = parms + " " + body;
            }
        }
        else if( src.PeekTokenString( "{" ) )
        {
            src.ParseBracedSectionExt( block.postfix, -1, true, '{', '}' );
            if( src.CheckTokenString( ";" ) )
            {
                block.postfix += ';';
            }
...
const char* vertexInsert_GLSL_1_20 =
{
    "#version 120\n"
    "#define PC\n"
...
const char* fragmentInsert_GLSL_1_20 =
{
    "#version 120\n"
    "#define PC\n"
    "#extension GL_EXT_gpu_shader4 : enable\n"
...
idStr ConvertCG2GLSL( const idStr& in, const char* name, bool isVertexProgram, idStr& uniforms )
{
    ...
    if( isVertexProgram )
    {
        switch( glConfig.driverType )
        {
            case GLDRV_OPENGL3X:
            {
                out.ReAllocate( idStr::Length( vertexInsert_GLSL_1_20 ) + in.Length() * 2, false );
                out += vertexInsert_GLSL_1_20;
                break;
            }
    ...
    else
    {
        switch( glConfig.driverType )
        {
            case GLDRV_OPENGL3X:
            {
                out.ReAllocate( idStr::Length( fragmentInsert_GLSL_1_20 ) + in.Length() * 2, false );
                out += fragmentInsert_GLSL_1_20;
                break;
            }
...
GLuint idRenderProgManager::LoadGLSLShader( GLenum target, const char* name, idList<int>& uniforms )
{
    ...
    switch( glConfig.driverType )
    {
        case GLDRV_OPENGL3X:
        {
            outFileGLSL.Format( "renderprogs/glsl-1.20/%s", name );
            outFileUniforms.Format( "renderprogs/glsl-1.20/%s", name );
            break;
        }
    ...
    else
    {
        ...
        if( lengthGLSL <= 0 )
        {
            common->Error( "GLSL file %s could not be loaded and may be corrupt", outFileGLSL.c_str() );
        }
        ...
        if( r_useUniformArrays.GetBool() )
        {
            ...
            if( lengthUniforms <= 0 )
            {
                common->Error( "uniform file %s could not be loaded and may be corrupt", outFileUniforms.c_str() );
            }
    ...
    if( r_useUniformArrays.GetBool() )
    {
        ...
        while( src.ReadToken( &token ) )
        {
            ...
            if( index == -1 )
            {
                common->Error( "couldn't find uniform %s for %s", token.c_str(), outFileGLSL.c_str() );
            }
